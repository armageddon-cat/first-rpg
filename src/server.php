...
//        var_dump('lamda'.$i++);
    ...
    if (in_array($socket, $read)) {//if there is new connection
        ...
        unset($read[array_search($socket, $read, true)]);//then remove from list available for read
    }
    // if any new messages
    if (!empty($read)) {
        $serverTime = DateTimeHelper::createDateTimeMicro();
        foreach ($read as $currentConnect) { //working out all connections
            ...
            if (!strlen($data)) { //connection was closed
                ...
                unset($connects[array_search($currentConnect, $connects, true)]);
